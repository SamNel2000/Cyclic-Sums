...
RepCombSSList = []  # Set of all found CombSS with duplicate values
...
for u in range(1, max): #Will look into effective ranges later
    C = u
    for v in range(1, max):
        D = v
        for w in range(1, max):
            if ((u + v + w) != (N - 3)): #If all the terms besides 1 & 2 does not equal N minus 3 it cannot be a CombSS.
                continue
            ...
            if set(DoneSet) == set(NSet):  # If the DoneSet of all the added terms includes the NSet then it can be added to a final list.
                ...
                if SortedTerms not in CombSSList:  # Checks if the tuple is already within the list, this is because multiple permuations of the same values.
                    CombSSList.append(SortedTerms) #Adds CombSS to a list if there are no repeat values within the tuple.
                    for num in terms:
                        if num in dup:
                            RepCombSSList.append(SortedTerms)  #Adds CombSS to a list with other Repeating value CombSS.
                        dup.add(num)
                    ...
                    if (SortedTerms in CombSSList) & (SortedTerms not in RepCombSSList):
                        ...
                        print(SortedTerms)
print("There are", len(CombSSList), "CombSS of order 4.\nThere are", len(NoRepCombSSList), "NoRepCombSS of order 4.")
