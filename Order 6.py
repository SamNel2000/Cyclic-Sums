...
for s in range(1, max): #Will look into effective ranges later
    C = s
        ...
        for u in range(1, max):
            ...
            for v in range(1, max):
                ...
                for w in range(1, max):
                    if ((s + t + u + v + w) != (N - 3)): #If all the terms besides 1 & 2 does not equal N minus 3 it cannot be a CombSS.
                        continue
...
print("There are", len(CombSSList), "CombSS of order 6.\nThere are", len(NoRepCombSSList), "NoRepCombSS of order 6.")
